...
st.markdown("ベイジアンA/Bテストを用いて、AとBの各コンバージョン率（CVR）に関する事後分布を算出しました。具体的には、BのCVRがAを上回る確率を評価し、どちらの施策がより効果的であるかの判断材料としています。事前分布としてベータ分布（α=1, β=1）を設定し、CVRの可能性を0から1の範囲で均一に仮定しています。")
